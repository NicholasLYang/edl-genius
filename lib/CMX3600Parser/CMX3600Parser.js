...
const REGEXP_PATTERNS = {
  CMX_TITLE_REGEX: /^TITLE:\s+(.+)\s*$/,
  CMX_FRAME_RATE_REGEX: /^FCM:\s+(.+)\s*$/,
...
class CMX3600Parser extends Transform {
  ...
  _transform(obj, enc, done = () => {}) {
    let line = obj.toString();
    // Remove BOM as it screws up regex
    line = line.replace(/^\uFEFF/, '');
    ...
    if (REGEXP_LINE_BEGINNINGS.CMX_EVENT_REGEX.test(line)) {
      ...
      if (parsedComment.comment) {
        if (this.currentEvent.comments) {
          this.currentEvent.comments.push(parsedComment.comment);
        } else {
          this.currentEvent.comments = [parsedComment.comment];
        }
      } else {
        Object.assign(this.currentEvent, parsedComment);
      }
    } else if (REGEXP_LINE_BEGINNINGS.CMX_FRAME_RATE_REGEX.test(line)) {
      const [, frameCodeMode] = REGEXP_PATTERNS.CMX_FRAME_RATE_REGEX.exec(line);
      this.frameCodeMode = frameCodeMode.trim();
      this.sourceFrameRate = changeFrameRate(line, this.sourceFrameRate);
    } else if (REGEXP_LINE_BEGINNINGS.CMX_TITLE_REGEX.test(line)) {
      const [, title] = REGEXP_PATTERNS.CMX_TITLE_REGEX.exec(line);
      this.title = title.trim();
    } else if (/\S/.test(line)) {
      // If there's at least one character of non-whitespace, add it to the comments
      if (this.currentEvent.comments) {
        this.currentEvent.comments.push(line);
      } else {
        this.currentEvent.comments = [line];
      }
    }
