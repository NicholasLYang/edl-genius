...
describe('CMX3600Parser', () => {
  ...
  describe('Comments', () => {
    ...
    it('addComment("* Misc") should add a comment property to Event', (done) => {
      ...
      output.on('data', data => results.push(data));
      output.on('end', () => {
        assert.strictEqual(results[0].comments[0], 'GETTY__QEVL1ESCP001_GREAT ESCAPES_MECKLENBURG SIX_AERIAL AROUND PRISON ON ALCATR');
        assert.strictEqual(results[0].comments[1], 'AZ ISLAND _622-21');
        done();
      });
    });
    ...
    it('addComment() should add exact text of passed string if is not an EDL comment', () => {
      ...
      assert.strictEqual(event.comments[0], '004  BFD_CHIL A10   C        01:00:01:05 01:00:02:05 01:00:11:19 01:00:12:19');
    });
