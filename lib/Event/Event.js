...
class Event {
  ...
  addComment(input) {
    ...
    if (Object.prototype.hasOwnProperty.call(parsedComment, 'sourceFile')) {
      ...
      this.comments.push(parsedComment.comment.trim());
    } else {
      this.comments = [parsedComment.comment.trim()];
    }
  }
  ...
  toObject() {
    ...
    if (this.comments) obj.comments = this.comments;
    if (this.toClip) obj.toClip = this.toClip;
