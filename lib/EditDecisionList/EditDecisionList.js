...
class EditDecisionList {
  ...
  async read(stream) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.parser.on('end', () => {
        if (this.parser.title) {
          this.title = this.parser.title;
        }
        if (this.parser.frameCodeMode) {
          this.frameCodeMode = this.parser.frameCodeMode;
        }
        resolve(this);
      });
  ...
  toObject() {
    const obj = {
      ...
      type: this.type,
      title: this.title,
      frameCodeMode: this.frameCodeMode,
    };
