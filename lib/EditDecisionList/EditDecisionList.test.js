...
const EditDecisionList = require('./EditDecisionList');
...
describe('EditDecisionList Class', () => {
  describe('Constructor', () => {
    ...
    it('Should set the recordStart and recordEnd framerates to this.frameRate', async () => {
      const edl = await new EditDecisionList(23.98).readFile('./test/edl_files/cmx3600.edl');
  ...
  describe('readFile', () => {
    it('Should find 9 events in test file "cmx3600.edl"', async () => {
      const edl = await new EditDecisionList(29.97).readFile('./test/edl_files/cmx3600.edl');
    ...
    it('Should find 588 events in test file "12_16 TL01 MUSIC.edl"', async () => {
      const edl = await new EditDecisionList(29.97).readFile('./test/edl_files/12_16 TL01 MUSIC.edl');
  ...
  describe('fromString', () => {
    it('Should find 9 events in text string from "cmx3600.edl"', async () => {
      ...
      await edl.fromString(edlstring);
  ...
  describe('fromBuffer', () => {
    it('should read from a buffer', async () => {
      ...
      const edl = await new EditDecisionList(29.97).fromBuffer(buf);
      assert.strictEqual(edl.events.length, 588);
      return true;
    });
  });
  ...
  it('Should correctly construct Events with comments and motion effects', async () => {
    /* 004  QEVL1GRN V     C        01:31:44:03 01:31:44:12 01:00:02:24 01:00:03:01
      ...
      */
    const edl = await new EditDecisionList(29.97).readFile('./test/edl_files/070816_EG101_HEISTS_ROUGH_CUT_SOURCES_PART 1.edl');
    ...
    assert.strictEqual(event.comments[0], 'GETTY IMAGES__QEVL1GRND130_UNDERGROUND_EL CHAPO TUNNELS_INTERIOR OF ALCATRAZ PRI')
    assert.strictEqual(event.comments[1], 'SON. ROW OF CELLS, CLOSE-UP OF CELL DOOR BARS, INSIDE OF JAIL CELL_180563302');
  });
  ...
  it('Should parse and store "TITLE: XXX"', async () => {
    const edl = await new EditDecisionList(29.97).readFile('./test/edl_files/070816_EG101_HEISTS_ROUGH_CUT_SOURCES_PART 1.edl');
    assert.strictEqual(edl.title, '070816_EG101_HEISTS_ROUGH_CUT_SOURCES.sub.01.sub.02');

    const edl2 = await new EditDecisionList(29.97).readFile('./test/edl_files/cmx3600.edl');
    assert.strictEqual(edl2.title, 'EDL Test SEQ');
  });

  it('Should parse and store "FCM: XXX"', async () => {
    const edl = await new EditDecisionList(29.97).readFile('./test/edl_files/070816_EG101_HEISTS_ROUGH_CUT_SOURCES_PART 1.edl');
    assert.strictEqual(edl.frameCodeMode, 'NON-DROP FRAME');

    const edl2 = await new EditDecisionList(29.97).readFile('./test/edl_files/cmx3600.edl');
    assert.strictEqual(edl2.frameCodeMode, 'DROP FRAME');
  });

  it('Should ignore "FCM: XXX" when frame rate is 24', async () => {
    const edl = await new EditDecisionList(24).readFile('./test/edl_files/pull001_201109_exr.edl');
